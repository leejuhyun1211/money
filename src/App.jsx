...
const SEED_ENTRIES = [
  { date: currentMonth+'-01', type:'income',  category:'월급',     amount:3500000, memo:'4월 월급',     payment_method:'계좌이체', is_fixed:true  },
  ...
  { date: currentMonth+'-14', type:'expense', category:'교통비',   amount:52000,   memo:'교통카드 충전',payment_method:'체크카드', is_fixed:false },
];
...
export default function App() {
  ...
  async function deleteItem(id, type) {
    ...
    setBudgets(prev => ({ ...prev, [month]: { ...prev[month], [key]: prev[month][key].filter(x => x.id !== id) } }));
  }
  ...
  const yearlyData = useMemo(() => {
    ...
    entries.forEach(e => {
      ...
      if (!map[m]) map[m] = { month:mLabel(m), 수입:0, 지출:0 };
      if (e.type==='income')  map[m].수입 += e.amount;
      if (e.type==='expense') map[m].지출 += e.amount;
    });
    return Object.keys(map).sort().map(k => ({...map[k], 잔액:map[k].수입-map[k].지출}));
  }, [entries]);
  const totalYearIncome  = yearlyData.reduce((s,d)=>s+d.수입,0);
  const totalYearExpense = yearlyData.reduce((s,d)=>s+d.지출,0);

  const openEntryModal = (preset={}) => {
    setModal('entry');
    setForm({ date:preset.date||new Date().toISOString().slice(0,10), type:preset.type||'expense', category:preset.category||'', amount:preset.amount||'', memo:preset.memo||'', payment_method:preset.payment_method||'현금', is_fixed:preset.is_fixed||false });
  };
  ...
  return (
    <div style={{ fontFamily:"'Apple SD Gothic Neo','Malgun Gothic',sans-serif", background:'#F5F2FA', minHeight:'100vh' }}>
      <div style={{ background:'#2D1B3D', display:'flex', alignItems:'stretch', padding:'0 20px', gap:'2px', overflowX:'auto' }}>
        <div style={{ color:'white', fontSize:'15px', fontWeight:'700', display:'flex', alignItems:'center', paddingRight:'16px', marginRight:'8px', borderRight:'1px solid #4A2D6A', whiteSpace:'nowrap' }}>
          💰 내 가계부
        </div>
        {TABS.map(([key,label]) => (
          <button key={key} onClick={() => setTab(key)} style={{
            ...
            padding:'13px 14px', fontSize:'12px', fontWeight:tab===key?'700':'400', cursor:'pointer', whiteSpace:'nowrap'
          }}>{label}</button>
        ))}
      </div>
      ...
      {tab === 'budget' && (
        <div style={{ padding:'18px', maxWidth:'1100px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', flexWrap:'wrap' }}>
            <select value={month} onChange={e => setMonth(e.target.value)} style={{ border:'1px solid #D9D0E8', borderRadius:'8px', padding:'6px 10px', fontSize:'13px', background:'white' }}>
              {MONTHS.map(m => <option key={m} value={m}>{mLabel(m)}</option>)}
            </select>
            <div style={{ marginLeft:'auto', display:'flex', gap:'8px' }}>
              <button onClick={() => { setModal('income'); setForm({ cat:'월급', amount:'' }); }} style={{ background:'#7C3AED', color:'white', border:'none', borderRadius:'8px', padding:'7px 14px', fontSize:'12px', fontWeight:'600', cursor:'pointer' }}>+ 수입</button>
              <button onClick={() => { setModal('expense'); setForm({ cat:'식비', amount:'' }); }} style={{ background:'#EC4899', color:'white', border:'none', borderRadius:'8px', padding:'7px 14px', fontSize:'12px', fontWeight:'600', cursor:'pointer' }}>+ 지출</button>
            </div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1.6fr 1fr', gap:'12px', marginBottom:'16px' }}>
            <div style={card}>
              ...
              <ResponsiveContainer width="100%" height={190}>
                <BarChart data={barData} margin={{ top:0, right:8, left:-15, bottom:36 }}>
                  ...
                  <Tooltip formatter={v=>[`${f(v)}원`]}/><Bar dataKey="예산" fill="#C4B5FD" radius={[3,3,0,0]}/><Bar dataKey="실제" fill="#7C3AED" radius={[3,3,0,0]}/>
            ...
            <div style={card}>
              ...
              <ResponsiveContainer width="100%" height={190}>
                <PieChart><Pie data={pieData} cx="50%" cy="46%" innerRadius={42} outerRadius={68} dataKey="value" nameKey="name">{pieData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip formatter={v=>[`${f(v)}원`]}/><Legend iconSize={7} wrapperStyle={{ fontSize:'9px' }}/></PieChart>
              </ResponsiveContainer>
            </div>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
            <div style={card}>
              <div style={{ fontSize:'12px', fontWeight:'600', color:'#6B5B7B', marginBottom:'10px' }}>💸 지출 예산</div>
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead><tr>{['항목','예산','실제','달성률',''].map((h,i)=><th key={i} style={{...TH,textAlign:i>0&&i<4?'right':'left'}}>{h}</th>)}</tr></thead>
                <tbody>
                  {data.expenses.map((e,i) => { const over=e.budget>0&&e.actual>e.budget; return <tr key={e.id}><td style={TD}><span style={{display:'inline-block',width:'7px',height:'7px',borderRadius:'50%',background:C[i%C.length],marginRight:'5px'}}/>{e.cat}</td><td style={{...TD,textAlign:'right',color:'#9B8FA0'}}>{f(e.budget)}</td><td style={{...TD,textAlign:'right',color:over?'#EF4444':'#2D1B3D',fontWeight:over?'700':'400'}}>{f(e.actual)}</td><td style={{...TD,textAlign:'right',minWidth:'80px'}}><ProgressBar val={e.actual} max={e.budget} color={over?'#EF4444':C[i%C.length]}/></td><td style={{...TD,textAlign:'center',width:'28px'}}><button onClick={()=>deleteItem(e.id,'expense')} style={{background:'none',border:'none',color:'#D1C4E9',cursor:'pointer',fontSize:'14px'}}>✕</button></td></tr>; })}
                  <tr style={{background:'#F5F2FA'}}><td style={{...TD,fontWeight:'700'}}>합계</td><td style={{...TD,textAlign:'right',fontWeight:'700'}}>{f(budgetEx)}</td><td style={{...TD,textAlign:'right',fontWeight:'700',color:totEx>budgetEx?'#EF4444':'#2D1B3D'}}>{f(totEx)}</td><td style={{...TD,textAlign:'right'}}><ProgressBar val={totEx} max={budgetEx} color="#7C3AED"/></td><td style={TD}/></tr>
                </tbody>
              </table>
            </div>
            <div style={card}>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead><tr>{['항목','목표','실제','달성률',''].map((h,i)=><th key={i} style={{...TH,textAlign:i>0&&i<4?'right':'left'}}>{h}</th>)}</tr></thead>
                <tbody>
                  {data.income.map(inc=><tr key={inc.id}><td style={TD}>{inc.cat}</td><td style={{...TD,textAlign:'right',color:'#9B8FA0'}}>{f(inc.budget)}</td><td style={{...TD,textAlign:'right',fontWeight:'600',color:'#7C3AED'}}>{f(inc.actual)}</td><td style={{...TD,textAlign:'right',minWidth:'80px'}}><ProgressBar val={inc.actual} max={inc.budget} color="#7C3AED"/></td><td style={{...TD,textAlign:'center',width:'28px'}}><button onClick={()=>deleteItem(inc.id,'income')} style={{background:'none',border:'none',color:'#D1C4E9',cursor:'pointer',fontSize:'14px'}}>✕</button></td></tr>)}
                  <tr style={{background:'#F5F2FA'}}><td style={{...TD,fontWeight:'700'}}>합계</td><td style={{...TD,textAlign:'right',fontWeight:'700'}}>{f(budgetIn)}</td><td style={{...TD,textAlign:'right',fontWeight:'700',color:'#7C3AED'}}>{f(totIn)}</td><td style={{...TD,textAlign:'right'}}><ProgressBar val={totIn} max={budgetIn} color="#7C3AED"/></td><td style={TD}/></tr>
                </tbody>
              </table>
            </div>
          </div>
      ...
      {tab === 'entries' && (
        <div style={{ padding:'18px', maxWidth:'1100px', margin:'0 auto' }}>
          ...
          <div style={card}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead><tr>{['날짜','구분','카테고리','금액','결제수단','메모','고정',''].map((h,i)=><th key={i} style={{...TH,textAlign:i===3?'right':'left'}}>{h}</th>)}</tr></thead>
              <tbody>
                ...
                {filteredEntries.map(e=>(
                  <tr key={e.id}>
                    ...
                    <td style={{...TD,textAlign:'center'}}><button onClick={()=>deleteEntry(e.id)} style={{background:'none',border:'none',color:'#D1C4E9',cursor:'pointer',fontSize:'14px'}}>✕</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ padding:'18px', maxWidth:'900px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px', flexWrap:'wrap', gap:'12px' }}>
            <div style={{ display:'flex', gap:'12px', flexWrap:'wrap' }}>
              {[{label:'월 고정 지출',val:fixedMonthly,color:'#EF4444'},{label:'고정항목 수',val:fixedEntries.length+'개',color:'#7C3AED',isText:true},{label:'연간 예상',val:fixedMonthly*12,color:'#F97316'}].map(({label,val,color,isText})=>(
                <div key={label} style={{...card,padding:'12px 20px',borderLeft:`4px solid ${color}`}}><div style={{fontSize:'11px',color:'#9B8FA0'}}>{label}</div><div style={{fontSize:'18px',fontWeight:'700',color}}>{isText?val:f(val)+'원'}</div></div>
              ))}
            </div>
          ...
          <div style={card}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead><tr>{['날짜','구분','카테고리','금액','결제수단','메모',''].map((h,i)=><th key={i} style={{...TH,textAlign:i===3?'right':'left'}}>{h}</th>)}</tr></thead>
              <tbody>
      ...
      {tab === 'goals' && (
        <div style={{ padding:'18px', maxWidth:'1100px', margin:'0 auto' }}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px', marginBottom:'16px' }}>
            {[{label:'목표 금액',val:`${f(totTarget)}원`,color:'#EF4444'},{label:'현재까지 확보',val:`${f(totSaved)}원`,color:'#22C55E'},{label:'남은 금액',val:`${f(totTarget-totSaved)}원`,color:'#F97316'},{label:'전체 진행률',val:`${overallPct.toFixed(1)}%`,color:'#3B82F6'}].map(({label,val,color})=>(
          ...
          <div style={{ display:'grid', gridTemplateColumns:'340px 1fr', gap:'12px' }}>
            <div style={card}>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead><tr><th style={TH}>날짜</th><th style={TH}>항목</th><th style={{...TH,textAlign:'right'}}>금액</th><th style={TH}></th></tr></thead>
                <tbody>
                  {txs.length===0&&<tr><td colSpan={4} style={{...TD,textAlign:'center',color:'#9B8FA0',padding:'24px'}}>내역 없음</td></tr>}
                  {txs.slice(0,15).map(tx=><tr key={tx.id}><td style={{...TD,color:'#9B8FA0',fontSize:'11px'}}>{tx.date}</td><td style={TD}>{tx.goal}</td><td style={{...TD,textAlign:'right',color:'#22C55E',fontWeight:'600'}}>{f(tx.amount)}</td><td style={{...TD,textAlign:'center',width:'28px'}}><button onClick={()=>deleteTx(tx)} style={{background:'none',border:'none',color:'#FCA5A5',cursor:'pointer',fontSize:'14px'}}>✕</button></td></tr>)}
                </tbody>
              </table>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', alignContent:'start' }}>
              {goals.map(g=>{
                const p=pct(g.saved,g.target), remain=g.target-g.saved;
      ...
      {tab === 'yearly' && (
        <div style={{ padding:'18px', maxWidth:'1100px', margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'16px' }}>
            <div style={card}>
              <div style={{fontSize:'12px',fontWeight:'600',color:'#6B5B7B',marginBottom:'12px'}}>📊 월별 수입/지출</div>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={yearlyData} margin={{top:0,right:8,left:-10,bottom:20}}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#F0EEF5"/><XAxis dataKey="month" tick={{fontSize:9,fill:'#9B8FA0'}} angle={-30} textAnchor="end" interval={0}/><YAxis tick={{fontSize:9,fill:'#9B8FA0'}} tickFormatter={v=>v>=10000?(v/10000)+'만':v}/><Tooltip formatter={v=>[`${f(v)}원`]}/>
                  <Bar dataKey="수입" fill="#C4B5FD" radius={[3,3,0,0]}/><Bar dataKey="지출" fill="#EC4899" radius={[3,3,0,0]}/><Legend wrapperStyle={{fontSize:'10px'}}/>
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div style={card}>
              <div style={{fontSize:'12px',fontWeight:'600',color:'#6B5B7B',marginBottom:'12px'}}>📈 월별 잔액 추이</div>
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={yearlyData} margin={{top:0,right:8,left:-10,bottom:20}}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#F0EEF5"/><XAxis dataKey="month" tick={{fontSize:9,fill:'#9B8FA0'}} angle={-30} textAnchor="end" interval={0}/><YAxis tick={{fontSize:9,fill:'#9B8FA0'}} tickFormatter={v=>v>=10000?(v/10000)+'만':v}/><Tooltip formatter={v=>[`${f(v)}원`]}/>
                  <Line type="monotone" dataKey="잔액" stroke="#7C3AED" strokeWidth={2} dot={{fill:'#7C3AED',r:4}}/>
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          <div style={card}>
            <div style={{fontSize:'12px',fontWeight:'600',color:'#6B5B7B',marginBottom:'12px'}}>📋 월별 요약</div>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr>{['월','수입','지출','잔액','저축률'].map((h,i)=><th key={i} style={{...TH,textAlign:i>0?'right':'left'}}>{h}</th>)}</tr></thead>
              <tbody>
                {yearlyData.map((d,i)=>{
                  const sr=d.수입>0?Math.round((d.잔액/d.수입)*100):0;
                  return <tr key={i}><td style={TD}>{d.month}</td><td style={{...TD,textAlign:'right',color:'#7C3AED',fontWeight:'600'}}>{f(d.수입)}</td><td style={{...TD,textAlign:'right',color:'#EC4899',fontWeight:'600'}}>{f(d.지출)}</td><td style={{...TD,textAlign:'right',fontWeight:'700',color:d.잔액>=0?'#059669':'#EF4444'}}>{f(d.잔액)}</td><td style={{...TD,textAlign:'right'}}><span style={sr>=20?bdg('#059669','#F0FDF4'):sr>=0?bdg('#F97316','#FFF7ED'):bdg('#EF4444','#FFF5F5')}>{sr}%</span></td></tr>;
                })}
              </tbody>
            </table>
          </div>
